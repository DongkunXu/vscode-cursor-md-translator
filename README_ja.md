...
| `mdTranslator.translationStyle` | `Professional` | 翻訳スタイル：`Professional` / `Legal` / `Financial` / `Engineering` / `Literary` / `🎭 Chaos Mode` |
...
| 起動方法 | 操作 |
|---|---|
| **ステータスバー / タイトルバー** | 右下のステータスバーにある `$(globe) Translate` ボタン、または編集エリア右上のタイトルバーにある地球アイコンをクリック |
